...
- (id)initWithURL:(NSURL *)newURL
{
    ...
    // Set the user-agent string for WMReader
    [self addRequestHeader:@"User-Agent" value:@"WMReader-iOS/0.1"];
...
{	
	...
	if ([self isResponseCompressed] && [self shouldWaitToInflateCompressedResponses]) {
		ret = [ASIDataDecompressor uncompressData:[self rawResponseData] error:NULL];
	} else {
		ret = [self rawResponseData];
	}
    
    // Rip invalid characters out of encoding GB18030-2000
    if ([[responseHeaders objectForKey:@"Content-Type"] rangeOfString:@"text"].location != NSNotFound) {        
        NSMutableData *data = [NSMutableData dataWithData:ret];
